...
- Пароли хранятся в открытом виде - **НЕ ИСПОЛЬЗУЙТЕ В ПРОДАКШЕНЕ**
- JWT токен простой (email + timestamp в Base64) - **НЕ БЕЗОПАСНО ДЛЯ ПРОДАКШЕНА**
- CORS разрешает все origins (`*`) - **ДОЛЖНО БЫТЬ ОГРАНИЧЕНО В ПРОДАКШЕНЕ**

**Эта конфигурация предназначена ТОЛЬКО для локальной разработки и обучения!**
